...
char *  SendCommand( unsigned char *hexbuffer )                                 
{
	...
	int wRet=0,i ;    
	int loc = 0,loctemp= 0, infoLen = 0,readTimes = 0; 
	char cmdbins[strlen(hexbuffer)/2+1],chtemp[3];
	unsigned  char    result[1024];//分配1024个字节的结果返回  
	static char ICRecvDataTEMP[2049];
	...
	wRet=write(fd_com, cmdbins, strlen(hexbuffer)/2);
		printf("SendCommand :" );
		for(i =0;i<strlen(hexbuffer)/2  ;i++)
		{
			printf(" %02x", cmdbins[i]);
		}
		printf("\n" );
   ...
   READ_CHAR:
	...
	loctemp = read(fd_com,result,   1024);  
	
	if( loctemp <=0 && readTimes < 100 ) 
	{		
		readTimes ++;
		goto READ_CHAR;
	}
	Bin2Hex(result,loctemp,ICRecvDataTEMP);	
	
	//printf("ICRecvDataTEMP=%s,   len=%d \n" ,ICRecvDataTEMP,loctemp*2 );
	strncpy(ICRecvData + loc*2, ICRecvDataTEMP,loctemp*2);	
	loc += loctemp;
	ICRecvData[loc*2 ] = 0x00;
	
	if (   infoLen = 0 && loc*2 > 5 )
	{
		chtemp[0]=ICRecvData[4];
		chtemp[1]=ICRecvData[5];
		chtemp[2]=0x00;
	        infoLen =  strtoul(chtemp,0,16);
	 }
	 printf("-->ICRecvData=%s,   infoLen=%d \n" ,ICRecvData, infoLen );
	 if(  loc*2 < 6 || ( strlen(ICRecvData + 6) < infoLen*2 &&  readTimes < 10 ) )
	 {
	    printf("ICRecvData=%s, %d len=%d \n" ,ICRecvData,loc*2,infoLen );
	    readTimes ++;
	    goto READ_CHAR;
	}
	...
	printf("ICRecvData =%s len=%d: \n" ,ICRecvData ,infoLen );
	/*printf("ICRecvData %d len=%d:" ,loc*2,infoLen );
	for(i =0;i<loc*2  ;i++)
	{
		printf(" %02x", ICRecvData[i]);
	}
	printf("\n" );*/
	//echoic("ICRecvData=%s\n",ICRecvData);
	return ICRecvData;                                                                                                   
...
char * _______ExcSAMCmd;                                                                                            
char* ExcSAMCmd(char * command)                                                                               
{
	static char  hexcmdSAM[1024];
	memset(hexcmdSAM,0x00,1024);
	 
	sprintf(hexcmdSAM,"%02X%02X%02X%02X%02X%02X%s",  0x55, 0xAA, strlen(command) / 2 + 3, 0x81, strlen(command)/ 2 , 0xFF, command);                                                              
	 
	_______ExcSAMCmd = SendCommand( hexcmdSAM);                                                                         
	                                                                                        
	return _______ExcSAMCmd;                                                                                                        
...
{
	static char  hexcmd[1024];
	memset(hexcmd,0x00,1024);	
	sprintf(hexcmd,"%02X%02X%02X%02X%02X%02X%02X%02X%02X%s",  0x55, 0xAA, strlen(command) / 2 + 6, 0x90, strlen(command)/ 2 + 4, 0x01, 0x06, (int)((cmdstate == T )? 0x0a : 0x0b), 0x00, command);                                                              
...
CPUResult * CPU_CommandEx(char * command)                                                             
{
	...
	printf("-->ExcCPUCmd ret=%s\n",strTemp);

	if ( strTemp!=NULL && strlen(strTemp)>0)
	{
        printf("-->CPU_CommandEx 处理返回值\n");
		if (StartsWith(strTemp,"55AA01")==T)
		{
			printf("-->CPU_CommandEx StartsWith 55AA01 \n");
			if (StartsWith(strTemp , "55AA010")==F)
			{
               printf("-->CPU_CommandEx StartsWith 55AA010  CPUResult.RADResult = T \n");
		...
		else //返回大于一个字节， 返回值无论如何，至少是俩字节。                                           
		{
			...
			printf("-->CPU_CommandEx StartsWith <> 55AA01, rLen=%d \n",(int)(rlen[0]) );
			if ((int)(rlen[0]) > 1)
			{
				int tt ;                     
				char crc[5];
...
char * GET_CHALLENGE_n(int len)
{
	...
	if (cr->PROResult==T)
	{
		printf("--> GET_CHALLENGE_n 取8字节随机数 is TRUE = %s\n",cr->PROResponse );
...
BOOL EXTERNAL_AUTHENTICATE_key(char * initkey)
{
	...
	if (sRand != NULL && strlen(sRand)==16)
	{
		...
		blTemp = USR_SelectSAM(1) ;//选择SAM卡 1
	        if( blTemp )
		{   //print_cpucscrhy("-->选择SAM卡1 TRUE \n"); 
		 }else
		{   //print_cpucscrhy("-->选择SAM卡1 FALSE \n");
		 }
		...
		if (cr->PROResult )
		{
			printf("--> EXTERNAL_AUTHENTICATE_key return is TRUE \n");
		}else
		{
            printf("--> EXTERNAL_AUTHENTICATE_key return is FALSE \n");
		}
		return cr->PROResult;
	}else
	{
...
BOOL SELECT(char * fileid)
{
	...
	memset(cmdtemp,0x00,32);
	//00A4 0000 021001
	sprintf(cmdtemp,"00A4000002%s",fileid);
	//sprintf(cmdtemp,"00A4000002%s",fileid);
	cr = CPU_CommandEx(cmdtemp ); 
	return cr->PROResult;
}
...
{
	//if(WF_IC_WriteBase() == true )
	if( 	ReadyCard(GetHexCardNumber()) == T ) 
	{
		printf("-->ReadyCard is true fileid=%s\n",fileid);

		if (strncmp(fileid,"0015",4)==0 || strncmp(fileid,"0019",4)==0 )
		{
			printf("-->fileid = %s \n",fileid);
			if (SELECT("1001")==F)
			{
...
BOOL ReadyCard(char * procard)
{
	...
	if ( 8==strlen(tempcard) &&  SelectPro(tempcard)==T)
	{
		...
		{
			cmdstate = T;
			echoic("--> ReadyCard return is TRUE \n");
			result = T;
		}
	}
	else
