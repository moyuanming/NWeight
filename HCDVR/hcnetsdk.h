...
#define NET_DVR_GET_TARFFIC_MASK_REGION     402     // 获取交通事件屏蔽区域
#define NET_DVR_SET_TARFFIC_MASK_REGION     403     //  设置交通事件屏蔽区域
...
/*********************************智能交通事件begin***************************************/
/*********************************车牌识别 begin***************************************/
#define NET_DVR_GET_PLATECFG            420         // 获取车牌识别参数
#define NET_DVR_SET_PLATECFG            421         // 设置车牌识别参数
...
#define IDS8104_AHL_S_H            67              // 海鑫人脸识别 ATM
...
#define DS_6500HF_B                 109             //65机架式DVR
...
{        
    ...
    BYTE	byJoinDecoderId[MAX_WINDOWS];/*各个子窗口对应解码通道所对应的解码子系统的槽位号*/
    BYTE	byRes[2];
...
{
    NET_VCA_POLYGON struRegion;//区域范围
    WORD wDuration;    //触发时间阈值: 1-100秒，建议5秒，判断是有效报警的时间
    BYTE byRes[6];
...
typedef struct tagNET_VCA_REACH_HIGHT
{
    ...
    WORD wDuration; //触发攀高报警阈值：1-100秒
    BYTE	byRes[6];           // 保留字节
...
// 人员滞留
typedef struct tagNET_VCA_TARRY
{
    NET_VCA_POLYGON struRegion; // 区域范围
    WORD wDuration; // 触发事件阈值 1-100s
    BYTE  bySensitivity;       //灵敏度参数，范围[1,5]
    BYTE   byRes[5];   // 保留字节
}NET_VCA_TARRY, *LPNET_VCA_TARRY;

//贴纸条参数
typedef struct tagNET_VCA_STICK_UP
{
    NET_VCA_POLYGON struRegion;//区域范围
    WORD wDuration; //触发时间阈值：10-50秒，建议10秒
    BYTE  bySensitivity;       //灵敏度参数，范围[1,5]
    BYTE byRes[5];
...
typedef struct tagNET_DVR_TRACK_PARAMCFG
{
    ...
    WORD    wTrackHoldTime;     // 报警跟踪持续时间 
    BYTE    byTrackMode;        //  参照 IPDOME_TRACK_MODE
    BYTE	 byPreDirection;	// 跟踪方向预判 
    BYTE 	 byTrackSmooth;	//跟踪连续 0 跟踪
    BYTE	 byZoomAdjust;	// 倍率系数调整 参见下表
    BYTE	byMaxTrackZoom;	// 最大跟踪倍率系数  0 表示默认倍率系数
    BYTE   byRes[11];          //  保留字节                
...
typedef struct tagNET_DVR_ONE_TPS_RULE
{
    ...
    BYTE    byCalcType;     // 统计参数类型ITS_TPS_TYPE
	BYTE	byLaneID;		// 车道ID
    BYTE    byRes1[1];
    NET_VCA_SIZE_FILTER struSizeFilter; // 尺寸过滤器 
...
typedef struct tagNET_DVR_LANE_PARAM
{
    ...
    BYTE    byVehicle;       //虚拟线圈内车辆标记 有车为1，无车为0 
	BYTE	byLaneType;			// 车道上行或下行
	BYTE	byRes1;
    DWORD    dwLaneVolume;	// 车道流量 ，统计有多少车子通过
    float   fLaneVelocity;      //车道速度，公里计算
    float   fTimeHeadway ;      //车头时距，以秒计算
    float   fSpaceHeadway;      //车头间距，以米来计算
...
//     NET_VCA_RECT	struPlateRect;		/* 车牌位置，使用归一化，小数点后3位 */
//     DWORD       dwRes[4];                /* 保留 */
//     DWORD    dwLicenseLen;					/* 车牌长度 */
//     char *pLicense;					/* 车牌号码 */
//     char *pBelieve;					/* 各个识别字符的置信度，如检测到车牌"浙A12345", 置信度为10,20,30,40,50,60,70，则表示"浙"字正确的可能性只有10%，"A"字的正确的可能性是20%*/	
...
//     DWORD      dwSize;     /*结构长度*/
//     //DWORD      dwChan;	/*通道号*/
//     DWORD     dwPlateNum;				    /* 车牌个数 */				        
//     LPNET_VCA_PLATE_INFO  pPALTE_PARAM;     /* 车牌信息结构 */
//     DWORD dwRes[4];						/*保留*/
//     NET_VCA_PICTURE_INFO struPictureInfo;			/* 返回图片 */
...
typedef struct tagNET_DVR_CODESYSTEMABILITY
{
    ...
    BYTE  byRes1[3];
    //分辨率0-DCIF 1-CIF, 2-QCIF, 3-4CIF, 4-2CIF 5（保留）,
    //16-VGA（640*480）, 17-UXGA（1600*1200）, 18-SVGA （800*600）,
    //19-HD720p（1280*720）,20-XVGA,  21-HD900p, 27-HD1080i, 
	//28-2560*1920, 29-1600*304, 30-2048*1536, 31-2448*2048	
    BYTE  byMainStreamSupportResolution[32];//主码流支持的分辨率（能力集1.1版本增加）
    BYTE  bySubStreamSupportResolution[32];//子码流支持的分辨率（能力集1.1版本增加）
    BYTE  byRes2[60];//保留
}NET_DVR_CODESYSTEMABILITY, *LPNET_DVR_CODESYSTEMABILITY;

//2010-03-31 增加视频综合平台能力集
typedef struct tagNET_DVR_SUBSYSTEM_ABILITY
{
	BYTE  bySubSystemType;//子系统类型，1-解码用子系统，2-编码用子系统，0-NULL
...
typedef struct tagNET_DVR_SNAPCFG
{
    ...
    WORD	wSnapWaitTime;  //抓拍等待时间，单位ms，取值范围[67,60000]
...
#define NET_DVR_GET_PD_LIST    1130    // 获取物理磁盘信息
#define NET_DVR_GET_ARRAY_LIST  1131    // 获取阵列信息
#define NET_DVR_GET_VD_LIST    1132    // 获取虚拟磁盘信息
#define NET_DVR_GET_ARRAY_SPACE_ALLOC   1133    // 获取阵列分配的虚拟磁盘信息
...
/******************************************
函数:	NET_DVR_RaidFastConfig
描述:	Raid一键配置  
输入:	lUserID: NET_DVR_Login()或NET_DVR_Login_V30的返回值 
        sName阵列名称  字符串大小为16字节
输出:	无
返回值: 1表示失败，其他值作为NET_DVR_FastConfigProcess等函数的参数。获取错误码调用NET_DVR_GetLastError
******************************************/
NET_DVR_API LONG __stdcall NET_DVR_RaidFastConfig(LONG lUserID, char *sName);

/******************************************
函数:	NET_DVR_FastConfigProcess
描述:	获取一键配置的进度
输入:	lHandle NET_DVR_RaidFastConfig返回值
        pState 指向状态指针
输出:	pState 指向状态指针
返回值: 成功-TRUE 失败-FALSE
******************************************/
...
/******************************************
函数:	NET_DVR_GetArraySpaceAlloc
描述:	获取阵列中空间分配信息
输入:	lUserID: NET_DVR_Login()或NET_DVR_Login_V30的返回值 
        dwSlot: 阵列的槽位
        lpOutBuf: 指向NET_DVR_ARRAY_SPACE_ALLOC_INFO结构指针
输出:	lpOutBuf: 指向NET_DVR_ARRAY_SPACE_ALLOC_INFO结构指针
返回值: 成功-TRUE 失败-FALSE
******************************************/
...
/******************************************
函数:	NET_DVR_CreateArray
描述:	创建Array
输入:	lUserID: NET_DVR_Login()或NET_DVR_Login_V30的返回值
        lpRaidParam 创建Array参数
输出:	无
返回值: 成功-TRUE 失败-FALSE
******************************************/
...
/******************************************
函数:	NET_DVR_CalcArraySize
描述:	估计创建Array的大小
输入:	lUserID: NET_DVR_Login()或NET_DVR_Login_V30的返回值
        lpRaidParam 创建Array参数
        lpArraySize 指向unsigned 结构指针
输出:	lpArraySize 指向unsigned 结构指针
返回值: 成功-TRUE 失败-FALSE
******************************************/
NET_DVR_API BOOL __stdcall NET_DVR_CalcArraySize(LONG lUserID, LPNET_DVR_OPERATE_ARRAY_PARAM lpArrayParam, unsigned long long* lpArraySize);

/******************************************
函数:	NET_DVR_MigrateArray
描述:	迁移Array
输入:	lUserID: NET_DVR_Login()或NET_DVR_Login_V30的返回值
        lpArrayParam 迁移Array参数
输出:	无
返回值: 成功-TRUE 失败-FALSE
******************************************/
...
/******************************************
函数:	NET_DVR_RebuildArray
描述:	重建Array    
输入:	lUserID: NET_DVR_Login()或NET_DVR_Login_V30的返回值
        lpArrayParam 重建Array参数
输出:	无
返回值: 成功-TRUE 失败-FALSE
******************************************/
...
/******************************************
函数:	NET_DVR_ExpandDisk
描述:	远程扩容虚拟硬盘
输入:	lUserID: NET_DVR_Login()或NET_DVR_Login_V30的返回值
        lVDSlot: 虚拟磁盘槽位，从1开始
输出:	无
返回值: -1表示失败，其他值作为NET_DVR_CloseExpandHandle等函数的参数。获取错误码调用NET_DVR_GetLastError
******************************************/
NET_DVR_API LONG __stdcall NET_DVR_ExpandDisk(LONG    lUserID,DWORD dwVDSLot);

/******************************************
函数:	NET_DVR_GetExpandProgress
描述:	获取扩容虚拟磁盘进度
输入:	lExpandHandle: 扩容虚拟磁盘句柄 NET_DVR_ExpandDisk返回值
		pState，扩容的返回进度
// 		0~100 表示进度	
// 		#define  PROCESS_SUCCESS     100   //过程完成
// 		#define  PROCESS_EXCEPTION   400   //过程异常
...
{
    BYTE	bySubSystemType;//子系统类型，0-无，1-解码用子系统，2-编码用子系统
    BYTE	byRes1[3];
...
typedef struct tagNET_DVR_VIDEOPLATFORMALARMCFG
{
    ...
    NET_DVR_HANDLEEXCEPTION_V30  struHandleException[MAX_EXCEPTIONNUM];//0-风扇异常，1-子系统异常，2-IP地址冲突，3-网线断，4-非法访问
    BYTE						 byRes[32];
...
{
	DWORD  dwSize;      //结构体大小
	BYTE   byAlarmType;	// 0-拔出子板，1-插入子板,2-温度超上限报警，3-温度超下限报警，4-风扇状态异常，5-子系统异常,6-非法访问
	BYTE   byRes1[3];
...
/*******************************test*****************************************/
NET_DVR_API LONG __stdcall NET_DVR_AlgoLibInfoOutput_Start(LONG lUserID, LONG lCHannel, void(CALLBACK *fAlgoLibInfoCallBack)(LONG lHandle, LONG lChannel,char *pRecvDataBuffer, DWORD dwBufSize, void* pUser), void* pUser);

NET_DVR_API BOOL __stdcall NET_DVR_AlgoLibInfoOutput_Stop(LONG lHandle);
